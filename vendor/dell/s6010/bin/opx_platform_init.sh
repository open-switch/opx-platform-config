...
for f in /sys/bus/i2c/devices/*; do
    if [ "`cat $f/name`" = 'i2c-0-mux (chan_id 0)' ]; then
        ...
        echo "System CPLD: $((`i2cget -y $b 0x31 8`))" >> $FIRMWARE_VERSION_FILE
        # Get Master CPLD version
        echo "Master CPLD: $((`i2cget -y $b 0x32 0x18`))" >> $FIRMWARE_VERSION_FILE
        # Get Slave CPLD version
        echo "Slave CPLD: $((`i2cget -y $b 0x33 0xb`))" >> $FIRMWARE_VERSION_FILE
        break
    fi
done
